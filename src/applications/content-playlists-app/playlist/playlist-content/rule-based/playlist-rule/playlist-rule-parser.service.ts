...
import { MetadataProfile } from 'app-shared/vmc-shared/custom-metadata/metadata-profile';
...
export class PlaylistRuleParserService implements OnDestroy {
  ...
  private _mapCustomMetadata(advancedSearch: VidiunSearchOperator): Observable<GroupedListType<string>> {
    if (!advancedSearch) {
    ...
    try {
      ...
      return this._getMetadataProfiles()
        .map(metadata => {
          const relevantItems = advancedSearch.items.filter((searchItem: VidiunMetadataSearchItem) => !!searchItem.items.length);
  ...
  public toEntriesFilters(rule: PlaylistRule): Observable<Partial<EntriesFilters>> {
    ...
    return this._mapCustomMetadata(<VidiunSearchOperator>originalFilter.advancedSearch)
      .map(customMetadata => {
  ...
  public toPlaylistRule(payload: { name: string, orderBy: VidiunPlayableEntryOrderBy, limit: number, rule: PlaylistRule }): Observable<PlaylistRule> {
    ...
    return this._entriesStore.convertFiltersToServerStruct()
      .map(originalFilter => {
        if (originalFilter instanceof VidiunMediaEntryFilterForPlaylist) {
          ...
          this._logger.error(`cannot build playlist rule. expected filter of type 'VidiunMediaEntryFilterForPlaylist'.`);
          throw new Error(`cannot build playlist rule. expected filter of type 'VidiunMediaEntryFilterForPlaylist'.`);
        }
      });
  }
}
